...
def send_ses_notification(user_email, stack_name, resource_name, resource_type, drift_time, aws_account_type, user_sso, federated_role, agent):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        recipient_account_id = event.get("recipientAccountId", "Unknown")
        aws_account_type = ACCOUNT_TYPE_MAPPING.get(recipient_account_id, "Unknown")
        ...
        if event_source == "aws.codepipeline":
            ...
            stack_name, resource_name, resource_type = pipeline_name, pipeline_name, "AWS::CodePipeline::Pipeline"

            drift_result, drift_time = check_drift(stack_name)
            if drift_result == "DRIFTED":
                send_ses_notification(
                    "user@example.com", stack_name, resource_name, resource_type,
                    drift_time, aws_account_type, user_sso, federated_role, agent
                )
